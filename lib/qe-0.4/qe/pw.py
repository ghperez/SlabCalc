# Developed by Prof. Roberto G. A. Veiga, Federal University of ABC, Brazil
# Version 0.4
...
class calc:
   ...
   * Please check https://www.quantum-espresso.org/Doc/INPUT_PW.html for a more
   ...
   read_input(inpfile) -> reads PW input file 'inpfile' and assigns values to attributes.
   build_input(saveinp,inpfile) -> returns PW input as a string. If 'saveinp' is set to 'True',
      the input script will be saved into the file 'inpfile'.
   run(command_line,input_string,saveout,outfile,savecoords,coordfile) -> runs 'command_line', 
      which should contain a path to PW executable, passing PW input in 'input_string', returning 
      an 'out' object. If 'saveout' is set to 'True', the PW output will be dumped into the
      file 'outfile'. If 'savecoords' is set to 'True', last atomic coordinates in the PW 
      output will be saved into file 'coordfile'.
   '''   
   ...
   def run(self,command_line,input_string,saveout=False,outfile="output",savecoords=False,coordfile="coord.xyz"):
      ...
      pw_out_obj=self.__get_output_info__(out,saveout,outfile,savecoords,coordfile)
   ...
   __write_input__(inpfile,input_string) -> writes PW input in 'input_string' into file 'inpfile'.
   __get_output_info__(output,saveout,outfile,savecoords,coordfile) -> retrieves information from 
      the output of a PW calculation to be stored in an 'out' object. If 'saveout' is set to 'True', 
      the output will be saved into file 'outfile'. If 'savecoords' is set to 'True', last atomic
      coordinates will be saved into 'coordfile'.
   ...
   def __write_coords__(self,coordfile,pw_out_obj):
      ...
      g.write('Lattice="%f %f %f %f %f %f %f %f %f" Properties=species:S:1:pos:R:3\n' \
